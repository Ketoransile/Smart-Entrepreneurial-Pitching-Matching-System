 ...
 * generate-openapi.ts
 *
 * Run with: tsx scripts/generate-openapi.ts
 *
 * Generates the full OpenAPI spec from JSDoc annotations in route files
 * and writes it to src/config/openapi-spec.json.
 *
 * This must run at BUILD time (when .ts source files are available),
 * before tsup bundles the project. tsup will then inline the JSON so
 * the Vercel runtime does not need to glob source files.
 */
...
const swaggerDefinition = {
	...
	servers: [
		{
			url: "https://sepms-backend.vercel.app",
...
// Point to source .ts route files (available at build time)
const routesSrc = path.resolve(process.cwd(), "src/routes/*.ts");
...
const outputPath = path.resolve(process.cwd(), "src/config/openapi-spec.json");
